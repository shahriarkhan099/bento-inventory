...
export async function searchConsumptionLogs (req: Request, res: Response) {
    try {
      ...
      if (searchTerm) {
        const consumptionLogs = await findConsumptionLogsByIngredientName(restaurantId, searchTerm);
        res.json({ consumptionLogs: consumptionLogs });
...
export async function postConsumptionLogToRestaurant (req: Request, res: Response) {
  try {
    const restaurantId = Number(req.params.restaurantId);
    if (restaurantId) {
      let consumptionLog = req.body;
      if (typeof consumptionLog.ingredientName === 'string') {
        const newConsumptionLog = await createConsumptionLogOfRestaurant(consumptionLog, restaurantId);
...
export async function putConsumptionLog (req: Request, res: Response) {
    try {
      ...
      if (consumptionLogId) {
        let consumptionLog = req.body;
        if (typeof consumptionLog.ingredientName === 'string') {
          const updatedConsumptionLog = await updateConsumptionLog(consumptionLogId, consumptionLog);
          res.json("Updated");
        } else {
          res.status(400).json({ message: "Invalid consumption log information." });
        }
...
export async function deleteConsumptionLogOfRestaurant (req: Request, res: Response) {
    try {
      const consumptionLogId = Number(req.params.consumptionLogId);
      if (consumptionLogId) {
        const deletedConsumptionLog = await deleteConsumptionLog(consumptionLogId);
        res.json("Deleted");
      } else res.status(400).json({ message: "Invalid consumption log ID." });
    } catch (error) {
      console.log(error);
