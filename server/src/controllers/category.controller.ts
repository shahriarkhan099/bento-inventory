...
import {
  findAllCategoryOfRestaurant,
  addCategoryToRestaurant,
  findCategoryBySearchTerm,
  updateCategoryOfRestaurant,
  deleteCategoryOfRestaurant,
} from "../models/category/category.query";
...
export async function searchCategory(req: Request, res: Response) {
  try {
    ...
    if (searchTerm) {
      const categories = await findCategoryBySearchTerm(
        restaurantId,
        searchTerm
      );
...
export async function updateCategory(req: Request, res: Response) {
  try {
    ...
    if (categoryId) {
      let category = req.body;
      if (typeof category.categoryName === "string") {
        const updatedCategory = await updateCategoryOfRestaurant(
          categoryId,
          category
        );
