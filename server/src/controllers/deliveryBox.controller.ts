...
import { findAllDeliveryBoxesOfRestaurant, findDeliveryBoxesByBoxName, createDeliveryBoxOfRestaurant, 
    updateDeliveryBox, deleteDeliveryBox } from "../models/deliveryBox/deliveryBox.query";
...
export async function getAllDeliveryBoxesOfRestaurant (req: Request, res: Response) {
    try {
      ...
      if (restaurantId) {
        const deliveryBoxes = await findAllDeliveryBoxesOfRestaurant(restaurantId);
...
export async function searchDeliveryBoxes (req: Request, res: Response) {
    try {
      ...
      if (searchTerm) {
        const deliveryBoxes = await findDeliveryBoxesByBoxName(restaurantId, searchTerm);
        res.json({ deliveryBoxes: deliveryBoxes });
  ...
  try {
    ...
    if (restaurantId) {
      ...
      if (typeof deliveryBox.boxName === 'string') {
        const newDeliveryBox = await createDeliveryBoxOfRestaurant(deliveryBox, restaurantId);
...
export async function putDeliveryBox (req: Request, res: Response) {
    try {
      ...
      if (deliveryBoxId) {
        let deliveryBox = req.body;
        if (typeof deliveryBox.boxName === 'string') {
          const updatedDeliveryBox = await updateDeliveryBox(deliveryBoxId, deliveryBox);
